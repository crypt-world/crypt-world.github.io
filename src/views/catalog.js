...
const catalogPageTemplate = (currencyPromise, onSearch, search = '', onPreviousPage, onNextPage) => html` <div
...
>
  <section class="searchbar-and-pagination">
    ...
    <section class="first-pagination">
      <button @click=${onPreviousPage} class="previous-button page-btn"><i class="fas fa-arrow-left"></i> Prev</button>
      <button @click=${onNextPage} class="next-button page-btn">Next <i class="fas fa-arrow-right"></i></button>
    </section>
  </section>
...
export  function catalogPage(ctx) {
  ...
  ctx.render(catalogPageTemplate(createCurrencyCard(page, search), onSearch, search, onPreviousPage, onNextPage));
...
function onSearch(event) {
 ...
 async function createCurrencyCard(page=1, search='') {
  const data = await getAllCryptoCurrencies(page, search);
